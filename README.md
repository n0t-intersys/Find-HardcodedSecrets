...
| 7 | **`intune/Detect-StoredCredentialSecrets.ps1`** | **OS/app credential stores** — saved Wi-Fi PSKs (plaintext), Windows Credential Manager, certificate private keys, browser saved-password stores, password-manager/key-store vault files, and service / scheduled-task command-line credentials | Seconds |
...
- **Structured (High):** provider-specific formats — AWS access key IDs, Google API/OAuth, Slack (tokens + webhook URLs), GitHub (classic + fine-grained PATs), GitLab, Stripe (live + test/restricted), SendGrid, Twilio (API key SID + Account SID), OpenAI/Anthropic, npm, Azure AD client secrets, Mailgun, PEM/OpenSSH/PGP private-key blocks, JWTs, Azure storage `AccountKey`, **URLs with embedded credentials** (`scheme://user:pass@host`), and (rev 3) Shopify, DigitalOcean, Doppler, Databricks, Grafana, Postman, Figma, Linear, Sentry, PyPI, Hugging Face, New Relic, Notion and Square tokens. The whole match is the token, so placeholder filtering is not applied.
- **Contextual (Medium):** keyword = value assignments — `password`/`passwd`/`passphrase`/`pwd`, `api_key`/`secret`/`secret_key`/`access_key`/`access_token`/`refresh_token`/`id_token`/`auth_token`/`client_secret`/`app_secret`/`session_key`/`token`, `Authorization: Bearer …`, **XML** secrets (`<password>…</password>`, `<add key="…Password" value="…"/>`, and unattend/sysprep `<…Password><Value>…`), **Group Policy Preferences** `cpassword="…"`, and connection strings with embedded credentials. The captured value is run through the placeholder + noise filters before recording.

**False-positive reduction:** contextual values matching placeholders/references (`${VAR}`, `%VAR%`, `{{VAR}}`, `<your-secret>`, `changeme`, leading `example`/`sample`/`redacted`, mask runs like `****`/`xxxxxx`, etc.) are dropped unless `-IncludePlaceholders` is set, as are obvious non-secrets (pure numbers, booleans, file paths, `sha256-`/`sha512-` integrity hashes). Combined with keyword-anchoring this keeps noise low even when scanning broad file types (e.g. `package-lock.json` integrity hashes are not flagged). For `.config`, findings near a `configProtectionProvider` marker are downgraded to **Low** and relabeled "encrypted config section (value protected)".

**Encoding (rev 3):** the file scanners are BOM-aware. UTF-16 / UTF-32 text — which is full of `NUL` bytes and was previously misclassified as *binary* and skipped entirely — is now detected (by BOM, and for BOM-less UTF-16 by the NUL parity pattern) and decoded correctly in both language modes. This matters on Windows because PowerShell's `Out-File` / `>` redirection default to UTF-16, so secrets written that way are no longer missed.
