...
- **Contextual (Medium):** keyword = value assignments — `password`/`passwd`/`passphrase`/`pwd`, `api_key`/`secret`/`secret_key`/`access_key`/`access_token`/`refresh_token`/`id_token`/`auth_token`/`client_secret`/`app_secret`/`session_key`/`token`, `Authorization: Bearer …`, **XML** secrets (`<password>…</password>` and `<add key="…Password" value="…"/>`), and connection strings with embedded credentials. The captured value is run through the placeholder + noise filters before recording.
